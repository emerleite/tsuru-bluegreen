...
class BlueGreen:
    ...
    def remove_units(self, app, units_to_keep=0):
        ...
        results = []
        for process_name, units in total_units.items():
  ...
  Removing %s '%s' units from %s ...""" % (units_to_remove, process_name, app))
        ...
        conn = create_connection(self.target)
        conn.request("DELETE", "/apps/" + app + '/units?units=' + str(units_to_remove) + '&process=' + process_name, '',
                     headers)
        response = conn.getresponse()
    ...
    def add_units(self, app, total_units_after_add):
        total_units = self.total_units(app)
        results = []
        for process_name, units in total_units_after_add.items():
            if total_units.get(process_name):
  ...
  Adding %s '%s' units to %s ...""" % (units_to_add, process_name, app))
        ...
        headers = {"Authorization": "bearer " + self.token}
        conn = create_connection(self.target)
        conn.request("PUT", "/apps/" + app + '/units?units=' + str(units_to_add) + '&process=' + process_name, '',
                     headers)
        response = conn.getresponse()
        response.read()
        if response.status != 200:
  ...
  Error running 'before' hook. Pre deploy aborted.
        ...
        for line in iter(process.stdout.readline, b''):
            sys.stdout.write(line if isinstance(line, str) else line.decode())
...
class Config:
    ...
    def load(self, filepath):
        ...
        return {'name': app_name, 'deploy_dir': deploy_dir, 'hooks': hooks, 'newrelic': newrelic, 'grafana': grafana,
                'webhook': webhook}
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--tag', metavar='TAG', help='Tag to be deployed (default: master)', nargs='?',
                        default="master")
