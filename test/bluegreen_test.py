...
from src.bluegreen import BlueGreen
...
class TestBlueGreen(unittest.TestCase):
    ...
    def test_env_set_return_true_when_can_set(self):
        httpretty.register_uri(httpretty.POST, 'http://tsuruhost.com/apps/xpto/env',
                               status=200)

        self.assertTrue(self.bg.env_set('xpto', 'TAG', 'tag_value'))

    @httpretty.activate
    def test_env_set_should_pass_noRestart(self):
        httpretty.register_uri(httpretty.POST, 'http://tsuruhost.com/apps/xpto/env',
                               status=200)

        self.bg.env_set('xpto', 'TAG', 'tag_value')

        request = httpretty.last_request()
        self.assertTrue("noRestart" in request.parsed_body)
        self.assertTrue('true' in request.parsed_body.get("noRestart"))
    ...
    def test_remove_units_should_return_true_when_removes_web_and_resque_units(self):
        self.bg.total_units = Mock(side_effect=self.mock_total_units(
            [{'web': 4, 'resque': 2}, {'web': 0, 'resque': 2}, {'web': 0, 'resque': 0}]))
    ...
    def test_remove_units_should_allow_keep_units(self):
        self.bg.total_units = Mock(side_effect=self.mock_total_units(
            [{'web': 4, 'resque': 2}, {'web': 1, 'resque': 2}, {'web': 1, 'resque': 1}]))
    ...
    def test_add_units_should_return_true_when_adds_web_and_resque_units(self):
        self.bg.total_units = MagicMock(side_effect=self.mock_total_units(
            [{'web': 2, 'resque': 1}, {'web': 5, 'resque': 1}, {'web': 5, 'resque': 2}]))
    ...
    def test_add_units_should_return_true_when_adds_only_web_units(self):
        self.bg.total_units = MagicMock(
            side_effect=self.mock_total_units([{'web': 2, 'resque': 1}, {'web': 5, 'resque': 1}]))
    ...
    def test_run_command_should_accept_environment_variables(self):
        ...
        env_test_file_path = os.path.join(workdir, 'test/env_test.sh')

        self.assertTrue(self.bg.run_command(env_test_file_path, {'VAR': '0'}))
        self.assertFalse(self.bg.run_command(env_test_file_path, {'VAR': '1'}))
    ...
    def test_deploy_pre_should_return_zero_when_success(self, subprocess_):
        ...
        subprocess_.return_value.stdout.readline.return_value = b''
